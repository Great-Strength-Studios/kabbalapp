...
def handle(context: MessageContext):
    ...
    domain_method = d.DomainMethod.create(**context.data.to_primitive())
    ...
    domain_model = domain_repo.get_domain_model(domain_method.parent_id)

    # Raise app error if the parent Domain Model does not exist.
    if not domain_model:
        raise AppError(context.errors.DOMAIN_MODEL_NOT_FOUND.format_message(domain_method.parent_id))
    ...
    if exists:
        raise AppError(context.errors.DOMAIN_METHOD_ALREADY_EXISTS.format_message(domain_method.name, domain_method.parent_id))
